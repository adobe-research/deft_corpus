...
def evaluate(y_gold, y_pred, eval_labels):
    ...
    """
    return classification_report(y_gold, y_pred, labels=eval_labels, target_names=eval_labels, output_dict=True)
...
def task_1_eval_main(ref_path, res_path, output_dir, eval_labels):
    ...
    for child in ref_path.iterdir():
        if child.name.startswith('task_1'):
            ...
            temp_y_gold, temp_y_pred = get_gold_and_pred_labels(child, res_path.joinpath(child.name))
